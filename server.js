...
async function handleDescriptionInput(chatId, userId, texto, userState) {
  ...
  try {
    ...
    const novoId = await db.criarEvento(novoEvento);
...
app.listen(PORT, async () => {
  ...
  if (conexaoOk) {
    console.log('🗄️ Banco de dados MySQL conectado com sucesso!');
  } else {
    console.log('⚠️ Aviso: Não foi possível conectar ao MySQL. Verifique as configurações.');
  }
});
